...
class ActiveOffer(TimestampMixin, Base):
    __tablename__ = "active_offers"

    id               = Column(String(36), primary_key=True, default=_uuid_default)
    offer_key        = Column(String, nullable=False, unique=True, index=True)  # e.g. deal_1, deal_2
    title            = Column(String, nullable=False)
    badge            = Column(String, nullable=True)   # e.g. ⚡ BEST VALUE, 🎉 50% OFF
    description      = Column(Text, nullable=True)
    discounted_price = Column(Float, nullable=False, default=0.0)
    original_price   = Column(Float, nullable=True, default=0.0)
    button_text      = Column(String, nullable=False)  # Button text for TG bot & Web UI
    is_active        = Column(Boolean, default=True, index=True)
    sort_order       = Column(Integer, default=0, index=True)
    items_json       = Column(Text, nullable=True)     # JSON config of included items/products
...
def seed_default_active_offers(db: Session):
    ...
    try:
        if db.query(ActiveOffer).count() > 0:
            return
    except Exception:
        return
    ...
    default_offers = [
        {
            "offer_key": "deal_1",
            "title": "🔥 2x Medium Non-Veg Pizza Combo",
            "badge": "⚡ BEST VALUE",
            "description": "2 Medium Non-Veg Pizzas of your choice for just ₹410!",
            "discounted_price": 410.0,
            "original_price": 650.0,
            "button_text": "🛒 Grab Deal 1 @ ₹410",
            "is_active": True,
            "sort_order": 1,
            "items_json": json.dumps([{"category": "Non-Veg", "size": "Medium", "qty": 2}])
        },
        {
            "offer_key": "deal_2",
            "title": "🎉 2x Medium Veg Pizza Combo",
            "badge": "🎉 POPULAR",
            "description": "2 Medium Veg Pizzas of your choice for just ₹320!",
            "discounted_price": 320.0,
            "original_price": 520.0,
            "button_text": "🛒 Grab Deal 2 @ ₹320",
            "is_active": True,
            "sort_order": 2,
            "items_json": json.dumps([{"category": "Veg", "size": "Medium", "qty": 2}])
        },
        {
            "offer_key": "deal_3",
            "title": "🍕 2x Regular Margherita Deal",
            "badge": "⭐ CLASSIC",
            "description": "2 Regular Margherita Pizzas for just ₹240!",
            "discounted_price": 240.0,
            "original_price": 350.0,
            "button_text": "🛒 Grab Deal 3 @ ₹240",
            "is_active": True,
            "sort_order": 3,
            "items_json": json.dumps([{"name": "Margherita", "size": "Regular", "qty": 2}])
        },
        {
            "offer_key": "deal_4",
            "title": "🌶️ 2x Regular Fresh Veggie Deal",
            "badge": "🌶️ SPICY DEALS",
            "description": "2 Regular Fresh Veggie (Paneer & Capsicum) for just ₹105!",
            "discounted_price": 105.0,
            "original_price": 220.0,
            "button_text": "🛒 Grab Deal 4 @ ₹105",
            "is_active": True,
            "sort_order": 4,
            "items_json": json.dumps([{"name": "Paneer", "qty": 1}, {"name": "Capsicum", "qty": 1}])
        },
        {
            "offer_key": "deal_5a",
            "title": "🧅 3x Onion Veggie Deal",
            "badge": "🧅 BUDGET SPECIAL",
            "description": "3 Single Topping Onion Pizzas for just ₹100!",
            "discounted_price": 100.0,
            "original_price": 180.0,
            "button_text": "🛒 Grab Deal 5A @ ₹100",
            "is_active": True,
            "sort_order": 5,
            "items_json": json.dumps([{"name": "Onion", "qty": 3}])
        },
        {
            "offer_key": "deal_5b",
            "title": "🍅 4x Classic Tomato/Capsicum Deal",
            "badge": "🍅 MEGA SAVER",
            "description": "4 Single Topping Classic Pizzas for just ₹90!",
            "discounted_price": 90.0,
            "original_price": 240.0,
            "button_text": "🛒 Grab Deal 5B @ ₹90",
            "is_active": True,
            "sort_order": 6,
            "items_json": json.dumps([{"name": "Classic", "qty": 4}])
        },
        {
            "offer_key": "deal_6",
            "title": "🍗 2x Chicken Sausage Special",
            "badge": "🍗 MEAT LOVER",
            "description": "2 Non-Veg Chicken Sausage Pizzas for just ₹105!",
            "discounted_price": 105.0,
            "original_price": 220.0,
            "button_text": "🛒 Grab Deal 6 @ ₹105",
            "is_active": True,
            "sort_order": 7,
            "items_json": json.dumps([{"name": "Chicken Sausage", "qty": 2}])
        }
    ]
    
    for offer_data in default_offers:
        offer = ActiveOffer(**offer_data)
        db.add(offer)
    try:
        db.commit()
    except Exception as e:
