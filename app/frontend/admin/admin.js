...
function renderOrderDetailPanel(order) {
  ...
  body.innerHTML = `
    ...
    <!-- Domino's & Store Details -->
    <div class="detail-section">
      <h4>Domino's & Store Details</h4>
      <div style="display:flex;flex-direction:column;gap:8px">
        <div>
          <label style="font-size:11px;color:var(--text-muted);display:block;margin-bottom:3px">Domino's Reference ID</label>
          <input type="text" id="detail-dominos-ref-input" class="form-control" placeholder="Domino's Ref (e.g. 65977-12345)" value="${order.dominos_reference || ''}" />
        </div>
        <div>
          <label style="font-size:11px;color:var(--text-muted);display:block;margin-bottom:3px">Sector Store Name</label>
          <input type="text" id="detail-sector-store-input" class="form-control" placeholder="Sector Store Name" value="${order.sector_store || ''}" />
        </div>
        <button class="btn btn-primary btn-sm" onclick="updateOrderDetails('${order.id}')">✏️ Update Details</button>
      </div>
    </div>
...
async function updateOrderDetails(orderId) {
  const dominos_reference = document.getElementById('detail-dominos-ref-input').value.trim();
  const sector_store = document.getElementById('detail-sector-store-input').value.trim();
  try {
    await adminFetch(`/admin/orders/${orderId}/details`, {
      method: 'PUT',
  ...
  body.innerHTML = `
    ...
    <div class="detail-section">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <span style="font-size:12px;color:var(--text-muted);font-weight:500">Change Role:</span>
          <select id="detail-role-select" class="form-control select-control" style="flex:1;height:32px;font-size:12px;padding:2px 8px">
            <option value="user" ${u.role === 'user' ? 'selected' : ''}>User</option>
            <option value="admin" ${u.role === 'admin' ? 'selected' : ''}>Admin</option>
          </select>
          <button class="btn btn-primary btn-sm" style="height:32px;padding:0 12px;font-size:12px" onclick="updateUserRoleFromDetail('${u.id}','${(u.display_name||'').replace(/'/g,"\\'")}')">👑 Apply</button>
        </div>
      </div>
    </div>
  `;
...
async function updateUserRoleFromDetail(userId, name) {
  const role = document.getElementById('detail-role-select').value;
  if (!confirm(`Change role for ${name} to ${role.toUpperCase()}?`)) return;
  try {
    await adminFetch(`/admin/users/${userId}/role`, {
      method: 'PUT',
      body: JSON.stringify({ role })
    });
    showToast(`Role updated to ${role.toUpperCase()}!`, 'success');
    openUserDetail(userId);
    await loadOrders();
  } catch (e) {
    showToast(e.message, 'error');
